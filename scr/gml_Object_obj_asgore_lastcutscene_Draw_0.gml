...
if (con == 18 && instance_exists(OBJ_WRITER) == 0)
{
    ...
    K = ini_read_real(("Flowey" + string(global.filechoice)), "K", 0)
    SK = ini_read_real(("Flowey" + string(global.filechoice)), "SK", 0)
    ossafe_ini_close()
    q = 0
    if (global.kills == 0)
    {
...
if (con == 106)
{
    ...
    ossafe_ini_open("undertale.ini")
    ini_write_real(("FFFFF" + string(global.filechoice)), "F", 1)
    ossafe_ini_close()
...
if (con == 121)
{
    ...
    if (won == 1)
    {
        ...
        ossafe_ini_open("undertale.ini")
        kill = ini_read_real(("Flowey" + string(global.filechoice)), "K", 0)
        if (kill == 1)
        {
...
{
    ossafe_ini_open("undertale.ini")
    ini_write_real(("Flowey" + string(global.filechoice)), "SK", 1)
    ossafe_ini_close()
    ossafe_savedata_save()
    global.typer = 63
    global.msc = 0
