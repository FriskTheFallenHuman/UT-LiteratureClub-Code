...
if (global.myfight == 2)
{
    if (whatiheard != -1)
    {
        if (global.heard == 0)
        {
            ...
            {
                global.msc = 0
                global.msg[0] = scr_gettext(("obj_torielboss_834" + string(char_name)))
                OBJ_WRITER.halt = 3
            ...
            if (whatiheard == 3)
            {
                tt += 1
                global.msc = 0
                global.msg[0] = scr_gettext(("obj_torielboss_843" + string(char_name)))
                ossafe_ini_open("undertale.ini")
                TK = ini_read_real(("Toriel" + string(global.filechoice)), "TK", 0)
                ossafe_ini_close()
                if (tt > 1)
                    global.msg[0] = scr_gettext(("obj_torielboss_847" + string(char_name)))
                if (tt > 2)
                    global.msg[0] = scr_gettext(("obj_torielboss_848" + string(char_name)))
                if (TK > 0)
                {
                    global.msg[0] = scr_gettext(("obj_torielboss_850" + string(char_name)))
                    global.msg[1] = scr_gettext("obj_torielboss_851")
                    global.msg[2] = scr_gettext(("obj_torielboss_852" + string(char_name)))
                }
                if (TK > 0 && tt > 1)
                    global.msg[0] = scr_gettext(("obj_torielboss_854" + string(char_name)))
                if (global.flag[202] >= 20)
                    global.msg[0] = scr_gettext(("obj_torielboss_855" + string(char_name)))
                OBJ_WRITER.halt = 3
                iii = instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER)
                with (iii)
                    halt = false
