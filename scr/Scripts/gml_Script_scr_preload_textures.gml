...
if ((room >= room_area1 && room <= room_truelab_power) || (room >= room_battle && room <= room_storybattle) || room == PLACE_FAILURE || room == room_papdate || room == room_adate || room == room_flowey_endchoice)
...
sprite_prefetch(spr_monikad)
if (room <= PLACE_MENU || (room >= room_settings && room <= room_controltest) || (room >= room_f_start && room <= room_f_room) || room == room_undertale_end || room == room_end_theend)
