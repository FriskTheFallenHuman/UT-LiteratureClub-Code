...
if (border_id == 3)
{
    ...
    if ((room_id >= room_area1 && room_id <= room_ruinsexit) || (room >= PLACE_LEGEND && room <= PLACE_MENU) || (room >= room_settings && room <= room_controltest))
        border_id = 4
...
if (border_id == 11)
{
    if (os_type == os_switch_beta || os_type == os_windows)
