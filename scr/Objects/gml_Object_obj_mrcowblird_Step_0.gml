...
{
    global.damage = 0
    instance_create(((x + (sprite_width / 2)) - 50), (y + 24), obj_dmgwriter)
    with (obj_dmgwriter)
        alarm[2] = 30
    global.myfight = 0
    global.mnfight = 1
    ...
    if (attacked == 0)
    {
        ...
        if (criticize == 0)
        {
            if (num == 1)
            {
                ...
                if (encourage > 0)
                    global.firingrate -= 2
                gen = instance_create(x, y, obj_bombgen)
            ...
            else
            {
                ...
                if (pop == 2)
                    global.firingrate = (global.firingrate * 1.7)
                gen = instance_create((((global.idealborder[0] + global.idealborder[1]) / 2) - 30), (global.idealborder[2] + 2), obj_bombgen)
                gen.dmg = global.monsteratk[myself]
            }
        }
        if (mycommand >= 0)
