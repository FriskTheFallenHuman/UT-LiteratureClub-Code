    ...
    if (whatiheard != -1)
    {
        if (global.heard == 0)
        {
            ...
            if (whatiheard == 1)
            {
                global.msc = 0
                script_execute(scr_mercystandard)
                global.msg[0] = scr_gettext(("obj_vegetoid_428" + string(char_name)))
                if (mercy < 8)
                {
                    global.msg[0] = scr_gettext(("obj_vegetoid_431" + string(char_name)))
            ...
            if (whatiheard == 4)
            {
                eat = 1
                global.msc = 0
                global.msg[0] = scr_gettext(("obj_vegetoid_445" + string(char_name)))
                OBJ_WRITER.halt = 3
                iii = instance_create(global.idealborder[0], global.idealborder[2], OBJ_WRITER)
                with (iii)
                    halt = false
